    ...
    fn read_from(raw_input: &Vec<u8>) -> WireCommands {
        ...
        match type_code {
            ...
            WrongHostCommand::TYPE_CODE => {
                WireCommands::WrongHost(WrongHostCommand::read_from(input))
            }
            SegmentIsSealedCommand::TYPE_CODE => {
                WireCommands::SegmentIsSealed(SegmentIsSealedCommand::read_from(input))
            }
            SegmentAlreadyExistsCommand::TYPE_CODE => {
                WireCommands::SegmentAlreadyExists(SegmentAlreadyExistsCommand::read_from(input))
            }
            SegmentIsTruncatedCommand::TYPE_CODE => {
                WireCommands::SegmentIsTruncated(SegmentIsTruncatedCommand::read_from(input))
            }
            NoSuchSegmentCommand::TYPE_CODE => {
                WireCommands::NoSuchSegment(NoSuchSegmentCommand::read_from(input))
            }
            TableSegmentNotEmptyCommand::TYPE_CODE => {
                WireCommands::TableSegmentNotEmpty(TableSegmentNotEmptyCommand::read_from(input))
            }
            InvalidEventNumberCommand::TYPE_CODE => {
                WireCommands::InvalidEventNumber(InvalidEventNumberCommand::read_from(input))
            }
            OperationUnsupportedCommand::TYPE_CODE => {
                WireCommands::OperationUnsupported(OperationUnsupportedCommand::read_from(input))
            }
            PaddingCommand::TYPE_CODE => WireCommands::Padding(PaddingCommand::read_from(input)),

            PartialEventCommand::TYPE_CODE => {
                WireCommands::PartialEvent(PartialEventCommand::read_from(input))
            }
            EventCommand::TYPE_CODE => WireCommands::Event(EventCommand::read_from(input)),
            SetupAppendCommand::TYPE_CODE => {
                WireCommands::SetupAppend(SetupAppendCommand::read_from(input))
            }
            AppendBlockCommand::TYPE_CODE => {
                WireCommands::AppendBlock(AppendBlockCommand::read_from(input))
            }
            AppendBlockEndCommand::TYPE_CODE => {
                WireCommands::AppendBlockEnd(AppendBlockEndCommand::read_from(input))
            }
            ConditionalAppendCommand::TYPE_CODE => {
                WireCommands::ConditionalAppend(ConditionalAppendCommand::read_from(input))
            }
            AppendSetupCommand::TYPE_CODE => {
                WireCommands::AppendSetup(AppendSetupCommand::read_from(input))
            }
            DataAppendedCommand::TYPE_CODE => {
                WireCommands::DataAppended(DataAppendedCommand::read_from(input))
            }
            ConditionalCheckFailedCommand::TYPE_CODE => WireCommands::ConditionalCheckFailed(
                ConditionalCheckFailedCommand::read_from(input),
            ),
            ReadSegmentCommand::TYPE_CODE => {
                WireCommands::ReadSegment(ReadSegmentCommand::read_from(input))
            }
            SegmentReadCommand::TYPE_CODE => {
                WireCommands::SegmentRead(SegmentReadCommand::read_from(input))
            }
            GetSegmentAttributeCommand::TYPE_CODE => {
                WireCommands::GetSegmentAttribute(GetSegmentAttributeCommand::read_from(input))
            }
            SegmentAttributeCommand::TYPE_CODE => {
                WireCommands::SegmentAttribute(SegmentAttributeCommand::read_from(input))
            }
            UpdateSegmentAttributeCommand::TYPE_CODE => WireCommands::UpdateSegmentAttribute(
                UpdateSegmentAttributeCommand::read_from(input),
            ),
            SegmentAttributeUpdatedCommand::TYPE_CODE => WireCommands::SegmentAttributeUpdated(
                SegmentAttributeUpdatedCommand::read_from(input),
            ),
            GetStreamSegmentInfoCommand::TYPE_CODE => {
                WireCommands::GetStreamSegmentInfo(GetStreamSegmentInfoCommand::read_from(input))
            }
            StreamSegmentInfoCommand::TYPE_CODE => {
                WireCommands::StreamSegmentInfo(StreamSegmentInfoCommand::read_from(input))
            }
            CreateSegmentCommand::TYPE_CODE => {
                WireCommands::CreateSegment(CreateSegmentCommand::read_from(input))
            }
            CreateTableSegmentCommand::TYPE_CODE => {
                WireCommands::CreateTableSegment(CreateTableSegmentCommand::read_from(input))
            }
            SegmentCreatedCommand::TYPE_CODE => {
                WireCommands::SegmentCreated(SegmentCreatedCommand::read_from(input))
            }
            UpdateSegmentPolicyCommand::TYPE_CODE => {
                WireCommands::UpdateSegmentPolicy(UpdateSegmentPolicyCommand::read_from(input))
            }
            SegmentPolicyUpdatedCommand::TYPE_CODE => {
                WireCommands::SegmentPolicyUpdated(SegmentPolicyUpdatedCommand::read_from(input))
            }
            MergeSegmentsCommand::TYPE_CODE => {
                WireCommands::MergeSegments(MergeSegmentsCommand::read_from(input))
            }
            MergeTableSegmentsCommand::TYPE_CODE => {
                WireCommands::MergeTableSegments(MergeTableSegmentsCommand::read_from(input))
            }
            SegmentsMergedCommand::TYPE_CODE => {
                WireCommands::SegmentsMerged(SegmentsMergedCommand::read_from(input))
            }
            SealSegmentCommand::TYPE_CODE => {
                WireCommands::SealSegment(SealSegmentCommand::read_from(input))
            }
            SealTableSegmentCommand::TYPE_CODE => {
                WireCommands::SealTableSegment(SealTableSegmentCommand::read_from(input))
            }
            SegmentSealedCommand::TYPE_CODE => {
                WireCommands::SegmentSealed(SegmentSealedCommand::read_from(input))
            }
            TruncateSegmentCommand::TYPE_CODE => {
                WireCommands::TruncateSegment(TruncateSegmentCommand::read_from(input))
            }
            SegmentTruncatedCommand::TYPE_CODE => {
                WireCommands::SegmentTruncated(SegmentTruncatedCommand::read_from(input))
            }
            DeleteSegmentCommand::TYPE_CODE => {
                WireCommands::DeleteSegment(DeleteSegmentCommand::read_from(input))
            }
            DeleteTableSegmentCommand::TYPE_CODE => {
                WireCommands::DeleteTableSegment(DeleteTableSegmentCommand::read_from(input))
            }
            SegmentDeletedCommand::TYPE_CODE => {
                WireCommands::SegmentDeleted(SegmentDeletedCommand::read_from(input))
            }
            KeepAliveCommand::TYPE_CODE => {
                WireCommands::KeepAlive(KeepAliveCommand::read_from(input))
            }
            AuthTokenCheckFailedCommand::TYPE_CODE => {
                WireCommands::AuthTokenCheckFailed(AuthTokenCheckFailedCommand::read_from(input))
            }
            UpdateTableEntriesCommand::TYPE_CODE => {
                WireCommands::UpdateTableEntries(UpdateTableEntriesCommand::read_from(input))
            }
            TableEntriesUpdatedCommand::TYPE_CODE => {
                WireCommands::TableEntriesUpdated(TableEntriesUpdatedCommand::read_from(input))
            }
            RemoveTableKeysCommand::TYPE_CODE => {
                WireCommands::RemoveTableKeys(RemoveTableKeysCommand::read_from(input))
            }
            TableKeysRemovedCommand::TYPE_CODE => {
                WireCommands::TableKeysRemoved(TableKeysRemovedCommand::read_from(input))
            }
            ReadTableCommand::TYPE_CODE => {
                WireCommands::ReadTable(ReadTableCommand::read_from(input))
            }
            TableReadCommand::TYPE_CODE => {
                WireCommands::TableRead(TableReadCommand::read_from(input))
            }
            ReadTableKeysCommand::TYPE_CODE => {
                WireCommands::ReadTableKeys(ReadTableKeysCommand::read_from(input))
            }
            TableKeysReadCommand::TYPE_CODE => {
                WireCommands::TableKeysRead(TableKeysReadCommand::read_from(input))
            }
            ReadTableEntriesCommand::TYPE_CODE => {
                WireCommands::ReadTableEntries(ReadTableEntriesCommand::read_from(input))
            }
            TableEntriesReadCommand::TYPE_CODE => {
                WireCommands::TableEntriesRead(TableEntriesReadCommand::read_from(input))
            }
            TableKeyDoesNotExistCommand::TYPE_CODE => {
                WireCommands::TableKeyDoesNotExist(TableKeyDoesNotExistCommand::read_from(input))
            }
            TableKeyBadVersionCommand::TYPE_CODE => {
                WireCommands::TableKeyBadVersion(TableKeyBadVersionCommand::read_from(input))
            }
