...
pub(crate) struct SegmentSelector {
    ...
    writers: HashMap<ScopedSegment, SegmentWriter>,
...
impl SegmentWriter {
    ...
    pub(crate) async fn setup_connection(
        ...
        {
            Ok(uri) => uri,
            Err(e) => return Err(SegmentWriter::RetryControllerWriting { err: e }),
        };
        ...
        .await
        {
            Ok((reply, connection)) => match reply {
                ...
                _ => {
                    ...
                    return Err(SegmentWriter::WrongReply {
                        expected: String::from("AppendSetup"),
                        actual: reply,
                    });
                }
            },
            Err(e) => return Err(SegmentWriter::RetryRawClient { err: e }),
        };
    ...
    pub(crate) async fn write(&mut self, event: PendingEvent) -> Result<(), SegmentWriter> {
    ...
    pub(crate) async fn write_pending_events(&mut self) -> Result<(), SegmentWriter> {
...
impl ServerReply {
    pub(crate) fn reply(&self) -> Replies {
        self.reply.clone()
    }
}
...
impl PendingEvent {
    ...
    pub(crate) fn new(
        ...
        if data.len() as i32 > PendingEvent::MAX_WRITE_SIZE {
            ...
            oneshot_sender
                .send(Err(SegmentWriter::EventSizeTooLarge {
    ...
    pub(crate) fn with_header(
        ...
        match cmd.write_fields() {
            ...
            Err(e) => {
                ...
                oneshot_sender
                    .send(Err(SegmentWriter::ParseToEventCommand { source: e }))
                    .expect("send error to caller");
                None
            }
        }
