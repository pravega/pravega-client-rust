...
impl ControllerClient for ControllerClientImpl {
    ...
    async fn create_stream(&self, stream_config: &StreamConfiguration) -> ResultRetry<bool> {
        call_controller_with_retry!(self, call_create_stream, stream_config)
    }

    async fn update_stream(&self, stream_config: &StreamConfiguration) -> ResultRetry<bool> {
        call_controller_with_retry!(self, call_update_stream, stream_config)
    }

    async fn truncate_stream(&self, stream_cut: &StreamCut) -> ResultRetry<bool> {
        call_controller_with_retry!(self, call_truncate_stream, stream_cut)
    }

    async fn seal_stream(&self, stream: &ScopedStream) -> ResultRetry<bool> {
        call_controller_with_retry!(self, call_seal_stream, stream)
    }

    async fn delete_stream(&self, stream: &ScopedStream) -> ResultRetry<bool> {
        call_controller_with_retry!(self, call_delete_stream, stream)
    }

    async fn get_current_segments(&self, stream: &ScopedStream) -> ResultRetry<StreamSegments> {
        call_controller_with_retry!(self, call_get_current_segments, stream)
    }

    async fn create_transaction(&self, stream: &ScopedStream, lease: Duration) -> ResultRetry<TxnSegments> {
        call_controller_with_retry!(self, call_create_transaction, stream, lease)
    }
    ...
    async fn ping_transaction(
        ...
        call_controller_with_retry!(self, call_ping_transaction, stream, tx_id, lease)
    }
    ...
    async fn commit_transaction(
        ...
        call_controller_with_retry!(self, call_commit_transaction, stream, tx_id, writer_id, time)
    }

    async fn abort_transaction(&self, stream: &ScopedStream, tx_id: TxId) -> ResultRetry<()> {
        call_controller_with_retry!(self, call_abort_transaction, stream, tx_id)
    }
    ...
    async fn check_transaction_status(
        ...
        call_controller_with_retry!(self, call_check_transaction_status, stream, tx_id)
    }

    async fn get_endpoint_for_segment(&self, segment: &ScopedSegment) -> ResultRetry<PravegaNodeUri> {
        call_controller_with_retry!(self, call_get_endpoint_for_segment, segment)
    }
    ...
    async fn get_successors(&self, segment: &ScopedSegment) -> ResultRetry<StreamSegmentsWithPredecessors> {
        call_controller_with_retry!(self, call_get_successors, segment)
    }
    ...
    async fn scale_stream(
        ...
        call_controller_with_retry!(self, call_scale_stream, stream, sealed_segment_ids, new_ranges)
    }

    async fn check_scale(&self, stream: &ScopedStream, scale_epoch: i32) -> ResultRetry<bool> {
        call_controller_with_retry!(self, call_check_scale, stream, scale_epoch)
    }
}
