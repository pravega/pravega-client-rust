...
pub trait FailingReplyProcessor: Send + Sync {
    ...
    fn segment_already_exists(&self, segment_already_exists: SegmentAlreadyExistsCommand)-> Result<(), ReplyError> {
    ...
    fn segment_is_truncated(&self, segment_is_truncated: SegmentIsTruncatedCommand)-> Result<(), ReplyError> {
    ...
    fn table_segment_not_empty(&self, table_segment_not_empty: TableSegmentNotEmptyCommand)-> Result<(), ReplyError> {
        Err(ReplyError::IllegalState {state: Replies::TableSegmentNotEmpty(table_segment_not_empty)})
    }

    fn invalid_event_number(&self, invalid_event_number: InvalidEventNumberCommand)-> Result<(), ReplyError> {
    ...
    fn conditional_check_failed(&self, data_not_appended: ConditionalCheckFailedCommand)-> Result<(), ReplyError> {
    ...
    fn segment_attribute_updated(&self, segment_attribute_updated: SegmentAttributeUpdatedCommand)-> Result<(), ReplyError> {
    ...
    fn operation_unsupported(&self, operation_unsupported: OperationUnsupportedCommand)-> Result<(), ReplyError> {
    ...
    fn segment_policy_updated(&self, segment_policy_updated: SegmentPolicyUpdatedCommand)-> Result<(), ReplyError> {
        Err(ReplyError::IllegalState {state: Replies::SegmentPolicyUpdated(segment_policy_updated)})
    }

    fn auth_token_check_failed(&self, auth_token_check_failed: AuthTokenCheckFailedCommand)-> Result<(), ReplyError> {
        Err(ReplyError::IllegalState {state: Replies::AuthTokenCheckFailed(auth_token_check_failed)})
    }

    fn table_entries_updated(&self, table_entries_updated: TableEntriesUpdatedCommand)-> Result<(), ReplyError> {
    ...
    fn table_key_does_not_exist(&self, table_key_does_not_exist: TableKeyDoesNotExistCommand)-> Result<(), ReplyError> {
        Err(ReplyError::IllegalState {state: Replies::TableKeyDoesNotExist(table_key_does_not_exist)})
    }

    fn table_key_bad_version(&self, table_key_bad_version: TableKeyBadVersionCommand)-> Result<(), ReplyError> {
