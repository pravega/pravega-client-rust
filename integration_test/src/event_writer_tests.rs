...
async fn test_write_correctness(writer: &mut EventWriter, factory: &ClientFactory) {
    ...
    let async_segment_reader = factory.create_segment_reader_wrapper(segment_name).await;
...
async fn test_write_correctness_while_scaling(writer: &mut EventWriter, factory: &ClientFactory) {
    ...
    let async_segment_reader = factory.create_segment_reader_wrapper(segment_name.clone()).await;
    let mut i: i32 = 0;
    let mut offset = 0;
    ...
    let reader1 = factory.create_segment_reader_wrapper(segment_name1.clone()).await;
    let segment_name2 = ScopedSegment {
        scope: scope_name.clone(),
        stream: stream_name.clone(),
        segment: Segment::from(4_294_967_298),
    };
    let reader2 = factory.create_segment_reader_wrapper(segment_name2.clone()).await;
...
async fn test_write_correctness_with_routing_key(writer: &mut EventWriter, factory: &ClientFactory) {
