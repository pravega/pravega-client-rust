...
pub fn test_byte_stream(config: PravegaStandaloneServiceConfig) {
    ...
    let mut writer = handle.block_on(client_factory.create_byte_writer(scoped_stream.clone()));
    let mut reader = handle.block_on(client_factory.create_byte_reader(scoped_stream));

    handle.block_on(test_simple_write_and_read(&mut writer, &mut reader));
    handle.block_on(test_seek(&mut reader));
    handle.block_on(test_truncation(&mut writer, &mut reader));
    handle.block_on(test_seal(&mut writer, &mut reader));
    ...
    let mut writer = handle.block_on(client_factory.create_byte_writer(scoped_stream.clone()));
    let mut reader = handle.block_on(client_factory.create_byte_reader(scoped_stream));
    handle.block_on(test_write_and_read_with_workload(&mut writer, &mut reader));
    ...
    handle.block_on(test_multiple_writers_conditional_append(
        &client_factory,
        scoped_stream,
    ));
}

async fn test_simple_write_and_read(writer: &mut ByteWriter, reader: &mut ByteReader) {
    ...
    assert_eq!(size1, 4);
    writer.flush().await.expect("flush byte stream writer");
    assert_eq!(writer.current_offset(), 4);

    let size2 = writer
        .write(&payload2)
        .await
        .expect("write payload2 to byte stream");
    assert_eq!(size2, 4);
    writer.flush().await.expect("flush byte stream writer");
    ...
    let bytes1 = reader.read(&mut buf).await.expect("read from byte stream");
    assert_eq!(bytes1, 4);
    assert_eq!(buf, payload1);

    let bytes2 = reader.read(&mut buf).await.expect("read from byte stream");
    assert_eq!(bytes2, 4);
...
async fn test_seek(reader: &mut ByteReader) {
    info!("test byte stream seek");
    // seek to start
    reader.seek(SeekFrom::Start(0)).await.expect("seek to start");
    let mut buf: Vec<u8> = vec![0; 4];
    let size = reader.read(&mut buf).await.expect("read from byte stream");
    assert_eq!(size, 4);
    assert_eq!(buf, vec![1; 4]);

    // seek to current
    reader.seek(SeekFrom::Current(-4)).await.expect("seek to current");
    let mut buf: Vec<u8> = vec![0; 4];
    let size = reader.read(&mut buf).await.expect("read from byte stream");
    assert_eq!(size, 4);
    assert_eq!(buf, vec![1; 4]);
    ...
    // seek to end
    reader.seek(SeekFrom::End(-4)).await.expect("seek to current");
    let mut buf: Vec<u8> = vec![0; 4];
    let size = reader.read(&mut buf).await.expect("read from byte stream");
    assert_eq!(size, 4);
...
async fn test_truncation(writer: &mut ByteWriter, reader: &mut ByteReader) {
    info!("test byte stream truncate");
    // truncate
    writer.truncate_data_before(4).await.expect("truncate");
    ...
    // get current head
    let head = reader.current_head().await.expect("get current head");
    assert_eq!(head, 4);

    // read from current head
    reader.seek(SeekFrom::Start(head)).await.expect("seek to start");
    let mut buf: Vec<u8> = vec![0; 4];
    let size = reader.read(&mut buf).await.expect("read from byte stream");
    assert_eq!(size, 4);
    assert_eq!(buf, vec![2; 4]);
...
async fn test_seal(writer: &mut ByteWriter, reader: &mut ByteReader) {
    info!("test byte stream seal");
    // seal
    writer.seal().await.expect("seal");

    // read sealed segment
    reader.seek(SeekFrom::Start(4)).await.expect("seek to start");
    let mut buf: Vec<u8> = vec![0; 4];
    let size = reader.read(&mut buf).await.expect("read from byte stream");
    assert_eq!(size, 4);
    assert_eq!(buf, vec![2; 4]);
...
async fn test_write_and_read_with_workload(writer: &mut ByteWriter, reader: &mut ByteReader) {
    info!("test write and read with workload");
    for _i in 0..10 {
        for _j in 0..1000 {
            let buf = vec![1; 1024];
            let size = writer.write(&buf).await.expect("write to byte stream");
            assert_eq!(size, 1024)
        }
...
async fn test_multiple_writers_conditional_append(
    ...
    let mut writer2 = factory.create_byte_writer(stream).await;
    writer2.seek_to_tail().await;
    let _num = writer2.write(&payload).await.expect("writer2 write payload");
    assert_eq!(writer2.current_offset(), 2048);
    writer2.flush().await.expect("writer2 flush");

    let writer_res = writer1.write(&payload).await;
    let flush_res = writer1.flush().await;
