...
use pravega_client::sync::synchronizer::{deserialize_from, Key, Synchronizer};
...
async fn test_insert(client_factory: &ClientFactory) {
    ...
    let mut synchronizer: Synchronizer = client_factory
        .create_synchronizer(scope.clone(), "synchronizer".to_owned())
        .await;

    let mut synchronizer2: Synchronizer = client_factory
        .create_synchronizer(scope.clone(), "synchronizer".to_owned())
        .await;
...
async fn test_remove(client_factory: &ClientFactory) {
    ...
    let mut synchronizer: Synchronizer = client_factory
        .create_synchronizer(scope.clone(), "synchronizer1".to_owned())
        .await;
    let result = synchronizer
        .insert(|table| {
            if table.is_empty() {
...
async fn test_insert_and_get_with_customize_struct(client_factory: &ClientFactory) {
    ...
    let mut synchronizer: Synchronizer = client_factory
        .create_synchronizer(scope.clone(), "synchronizer2".to_owned())
        .await;
...
async fn test_fetching_updates_delta(client_factory: &ClientFactory) {
    ...
    let mut synchronizer: Synchronizer = client_factory
        .create_synchronizer(scope.clone(), "fetch_updates".to_owned())
        .await;
